#!/usr/bin/env python3
"""One house v5: pure shingle roof + separate cream/timber facade.

No 901..918 macro roof pieces. The roof is constructed from pure current
shingle surface/cut pieces, so wall sprites remain a separate visual layer.
"""
...
ORIGIN=(5,5);SCENE_W=13;SCENE_H=12;WORLD_DELTA=(1,0,-1)
W=3;H=3

# Pure shingle family, no embedded walls.
SURFACE_N=178
SURFACE_BODY=179
SURFACE_S=181
WEST_HALF={178:482,179:479,181:481}
EAST_HALF={178:486,179:483,181:485}
...
def build_lower():
 ...
 east_mid=(ox+W-1,oy+1);south_left=(ox,oy+H-1);south_mid=(ox+1,oy+H-1);se=(ox+W-1,oy+H-1)
 for pos in sorted(topo,key=lambda p:(p[1],p[0])):
  role=HALF[cardinal(topo,*pos)];cid=BASE_WALL[role];sem=f"structural_{role}"
  if pos[0]==ox+W-1 and pos!=se:
   cid=FACADE["east_window"] if pos==east_mid else FACADE["east_wall"]
   sem="east_window" if pos==east_mid else "east_timber_wall"
  if pos[1]==oy+H-1:
   if pos==south_left:cid=FACADE["south_window"];sem="south_window"
   elif pos==south_mid:cid=FACADE["south_door"];sem="south_door"
   elif pos==se:cid=FACADE["se_corner"];sem="south_east_corner"
   else:cid=FACADE["south_wall"];sem="south_timber_wall"
  lower[pos].append(cid);roles[pos]=sem
 return lower,roles

def row_surface(y):
 return SURFACE_N if y==0 else SURFACE_S if y==H-1 else SURFACE_BODY

def build_upper():
 ox,oy=ORIGIN;rx=ox+WORLD_DELTA[0];ry=oy+WORLD_DELTA[1]
 upper={};placements=[]
 for y in range(H):
  parent=row_surface(y)
  # side eaves are cut derivatives of the same row surface
  west=(rx-1,ry+y);east=(rx+W,ry+y)
  upper[west]=[WEST_HALF[parent]]
  upper[east]=[EAST_HALF[parent]]
  placements.append({"screen_role":f"row{y}_west_edge","client_id":WEST_HALF[parent]})
  for x in range(W):
   upper[(rx+x,ry+y)]=[parent]
   role=("north" if y==0 else "south" if y==H-1 else "fill")
   placements.append({"screen_role":f"{role}_{x}","client_id":parent})
  placements.append({"screen_role":f"row{y}_east_edge","client_id":EAST_HALF[parent]})
 return upper,placements

def emit(lower,upper,c2s):
 props=u8(A_DESC)+s("PP-MAP one house v5 pure shingle roof")
...
def render(pos,items,sprites,path,close=False):
 ...
 if not close:
  d.text((8,8),"ONE HOUSE V5 — pure shingle roof / separate facade",fill=(240,240,240,255),font=font)
  d.text((8,25),"NO macro roof sprites with embedded walls",fill=(185,192,202,255),font=font)
  base=(255,175)
 else:base=(115,85)
...
def main():
 ...
 req={GROUND,SURFACE_N,SURFACE_BODY,SURFACE_S,*WEST_HALF.values(),*EAST_HALF.values(),*BASE_WALL.values(),*FACADE.values()}
 c2s,s2c=maps(a.otsp,req)
 lower,roles=build_lower();upper,placements=build_upper();payload=emit(lower,upper,c2s);pos=inspect(payload,s2c)
 ...
 if any(cid>=901 and cid<=918 for st in upper.values() for cid in st):raise RuntimeError("macro roof leaked into pure roof")
 otbm=a.out/"one_house_v5.otbm";otbm.write_bytes(payload)
 _,items=parse_all_items(a.otsp/"client_files"/"otsp.dat");sprites=_Sprites(a.otsp/"client_files"/"otsp.spr")
 render(pos,items,sprites,a.out/"one_house_v5.png",False)
 render(pos,items,sprites,a.out/"one_house_v5_close.png",True)
 report={
  "result":"PASS","proof_key":"ONE_HOUSE_V5_PURE_SHINGLE",
  "scope":"ONE_HOUSE_ONLY",
  "macro_roof_901_918_used":False,
  "roof":{"north_surface":178,"body_surface":179,"south_surface":181,"west_half":WEST_HALF,"east_half":EAST_HALF,"placements":placements},
  "wall":BASE_WALL,"facade":FACADE,"roles":{f"{x},{y}":v for (x,y),v in roles.items()},
  "otbm":{"file":otbm.name,"bytes":len(payload),"sha256":hashlib.sha256(payload).hexdigest(),"exact_parseback":True},
  "render":{"file":"one_house_v5.png","close":"one_house_v5_close.png","exact_current_dat_spr":True,"image_generation_used":False},
