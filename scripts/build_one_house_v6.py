...
ORIGIN=(5,5);SCENE_W=13;SCENE_H=12;WORLD_DELTA=(1,0,-1);W=3;H=3
...
def emit(lower,upper,c2s):
 props=u8(A_DESC)+s("PP-MAP one house v6 brown shingle roof")
...
def render(pos,items,sprites,path,close=False):
 ...
 if not close:
  d.text((8,8),"ONE HOUSE V6 — brown shingle roof / cream timber facade",fill=(240,240,240,255),font=font)
  d.text((8,25),"roof family 189/190 + south edge 508; no macro roof",fill=(185,192,202,255),font=font)
  base=(255,175)
 else:base=(115,85)
...
def main():
 ...
 otbm=a.out/"one_house_v6.otbm";otbm.write_bytes(payload)
 _,items=parse_all_items(a.otsp/"client_files"/"otsp.dat");sprites=_Sprites(a.otsp/"client_files"/"otsp.spr")
 render(pos,items,sprites,a.out/"one_house_v6.png",False)
 render(pos,items,sprites,a.out/"one_house_v6_close.png",True)
 report={
  "result":"PASS","proof_key":"ONE_HOUSE_V6_BROWN_SHINGLE",
  "scope":"ONE_HOUSE_ONLY","macro_roof_901_918_used":False,
 ...
 (a.out/"one_house_v6.json").write_text(json.dumps(report,indent=2,sort_keys=True)+"\n")
