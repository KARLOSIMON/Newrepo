...
ORIGIN=(5,5);SCENE_W=13;SCENE_H=12;WORLD_DELTA=(0,0,-1);W=3;H=3
...
def emit(lower,upper,c2s):
 props=u8(A_DESC)+s("PP-MAP one house v7 brown shingle roof shifted left")
...
def render(pos,items,sprites,path,close=False):
 ...
 if not close:
  d.text((8,8),"ONE HOUSE V7 — roof shifted exactly one tile left",fill=(240,240,240,255),font=font)
  d.text((8,25),"V6 unchanged except entire roof X - 1",fill=(185,192,202,255),font=font)
  base=(255,175)
 else:base=(115,85)
...
def main():
 ...
 otbm=a.out/"one_house_v7.otbm";otbm.write_bytes(payload)
 _,items=parse_all_items(a.otsp/"client_files"/"otsp.dat");sprites=_Sprites(a.otsp/"client_files"/"otsp.spr")
 render(pos,items,sprites,a.out/"one_house_v7.png",False)
 render(pos,items,sprites,a.out/"one_house_v7_close.png",True)
 report={
  "result":"PASS","proof_key":"ONE_HOUSE_V7_ROOF_SHIFTED_LEFT",
