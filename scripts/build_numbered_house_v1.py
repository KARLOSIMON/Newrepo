...
def emit(lower,upper,c2s)->bytes:
    props=u8(A_DESC)+s("PP-MAP numbered house v1 - literal user 1-16 assembly")
...
def main()->None:
    ...
    otbm=args.out/"numbered_house_v1.otbm"
    ...
    render(positions,items,sprites,args.out/"numbered_house_v1.png",False)
    render(positions,items,sprites,args.out/"numbered_house_v1_debug.png",True)

    report={
        "project":"PP-MAP-001",
        "result":"PASS",
        "proof_key":"USER_NUMBERED_HOUSE_1_16_V1",
    ...
    (args.out/"numbered_house_v1.json").write_text(json.dumps(report,indent=2,sort_keys=True)+"\n")
